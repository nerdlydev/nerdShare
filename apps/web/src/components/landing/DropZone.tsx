import { PlusIcon } from "@/components/ui/plus-icon";
import { useTranslation, Trans } from "react-i18next";

interface DropZoneProps {
  isDragOver: boolean;
  zipping: boolean;
  zipProgress: {
    filesProcessed: number;
    totalFiles: number;
  } | null;
  onDrop: (e: React.DragEvent) => void;
  onDragOver: (e: React.DragEvent) => void;
  onDragLeave: () => void;
  onClick: () => void;
  onFolderClick: (e: React.MouseEvent) => void;
  className?: string;
}

export function DropZone({
  isDragOver,
  zipping,
  zipProgress,
  onDrop,
  onDragOver,
  onDragLeave,
  onClick,
  onFolderClick,
  className = "",
}: DropZoneProps) {
  const { t } = useTranslation();

  return (
    <div
      className={`relative w-full ${className}`}
    >
      <div
        onDrop={onDrop}
        onDragOver={onDragOver}
        onDragLeave={onDragLeave}
        onClick={onClick}
        className={`
          relative w-full border-2 border-dashed rounded-[2.5rem]
          flex flex-col items-center justify-center gap-5 py-16 px-6
          transition-all duration-300 bg-card/60 backdrop-blur-md cursor-pointer shadow-sm hover:shadow-md
          min-h-[280px] sm:min-h-[320px] group
          ${
            zipping
              ? "border-primary/50 cursor-wait"
              : isDragOver
                ? "border-primary bg-primary/10 cursor-copy scale-[1.02]"
                : "border-border hover:border-primary/50 hover:bg-card/80"
          }
        `}
      >
        {zipping ? (
          <div className="flex flex-col items-center gap-5">
            <div className="w-10 h-10 border-3 border-primary border-t-transparent rounded-full animate-spin" />
            <p className="text-base text-muted-foreground font-medium text-balance text-center">
              {zipProgress && zipProgress.totalFiles > 0
                ? t('dropzone.zippingProgress', { processed: zipProgress.filesProcessed, total: zipProgress.totalFiles })
                : t('dropzone.preparing')}
            </p>
          </div>
        ) : (
          <>
            <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary/20 group-hover:scale-110 transition-all shadow-sm">
              <PlusIcon size={32} />
            </div>
            <div className="text-center space-y-2 max-w-sm mx-auto">
              <p className="text-lg sm:text-xl font-medium text-foreground text-balance leading-tight">
                {t('dropzone.title')}
              </p>
              <p className="text-base text-muted-foreground text-balance">
                <Trans 
                  i18nKey="dropzone.folderOption"
                  components={{
                    action: (
                      <span
                        onClick={onFolderClick}
                        className="text-primary hover:underline cursor-pointer font-medium relative top-[-1px]"
                      />
                    )
                  }}
                />
              </p>
              <p className="text-sm text-muted-foreground/70 pt-2 font-medium text-balance">
                {t('dropzone.footer')}
              </p>
            </div>
          </>
        )}
      </div>
    </div>
  );
}
